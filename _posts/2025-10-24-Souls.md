---
layout: single # post
classes: wide
title:  "Souls"
post-order: 4  
categories:
  - game
---

This is a multiplayer game inspired by the Inuyasha Demon Tournament. 
<br/>
Currently, multiplayer is limited to local host (LAN), so only two devices on the same network can connect.
<br/>
There's a bug, so if you press "Quit" once, you have to quit the game and restart it.
<br/>
It's distributed as a file for Windows and Android. 
You can install it by following the instructions (installation, etc.) in the GitHub readme.md file linked to the link.

[Download page][gamelink]

[gamelink]: https://github.com/blueSparrow2000/Souls_Build

[Youtube][tubelink]

[tubelink]: https://youtu.be/oHW0zudMw8k


이누야샤 데몬 토너먼트를 모티브로 만든 멀티플레이어 게임입니다.       
현재 local host (LAN) 으로만 멀티플레이를 즐길 수 있어서 같은 네트워크상의 두 기기만 접속할 수 있습니다. (서버비가 너무 비싸요 ㅠㅠ)        
버그가 있어서 한번 Quit을 누르면 게임을 종료하고 다시 실행해야 합니다.     

<br/>
Windows용 및 Android용 파일로 배포하였습니다.         
링크의 Github readme.md에 올라와있는 안내사항 (installation 등) 을 따라서 설치하시면 됩니다.      

<br/>
후기: C#, UNITY, Netcode(로컬 멀티 기능)로 2주만에 빠르게 개발하려다 보니까 코드가 중구난방이라 아쉬운 점이 있습니다.        
특히 Netcode 사용이 익숙하지 않아 quit을 해서 connection이 끊기면 게임을 재실행해야 하는데, 이 점을 나중에 고치면 좋을 것 같습니다.       


<p align="center">
</p>
