...
이렇게 얻은 확산모델은 사용자의 입력을 바탕으로 노이즈를 예측하여 원하는 이미지를 생성하는데 쓸 수도 있다. 훈련을 마치고 생성하는 단계에서는 텍스트 입력을 바탕으로 랜덤한 노이지 이미지에서 노이즈를 제거하면서 이미지를 만든다!
...
GLIDE라는 모델은 분류기 없이 단계(step)정보에 텍스트 정보를 추가로 이용해 학습한다.
...
DDPM을 사용하면 하나의 확산모델로도 노이즈가 제거된 고품질 이미지를 생성할 수 있으나, 이미지가 커지면 성능이 하락한다.
