...
# noise가 있는 경우
이때는 코드워드에 noise bit이 추가되므로 코드워드 행렬의 차원이 k보다 커질 수 있다.
여기에 ECO를 적용하면 lower triangular matrix(LTM)이 만들어지며, 0column이 없을 수 있다.
noise가 충분히 작다면 적절한 threshold를 정하여 1의 개수가 그것보다 작으면 0행렬로 보는 것이다.
이때도 code word들을 0으로 만드는 대응되는 ECO행렬의 column들이 parity check vector후보라고 볼 수 있다.

다만 이때 구해진 벡터들이 100%일치하지는 않을 수 있다.
